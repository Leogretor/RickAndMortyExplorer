...
- `presentation/characters/`
  ...
  - `CharactersUiState.kt`: sealed screen state with `UiText`-backed errors
  - `CharactersViewModel.kt`: loads the character list and exposes a `StateFlow`
- `presentation/characterdetail/`
  - `CharacterDetailScreen.kt`: stateless detail UI with a Material `TopAppBar`
  - `CharacterDetailUiState.kt`: sealed screen state with `UiText`-backed errors
...
- `presentation/common/`
  ...
  - `ScreenStateContent.kt`: reusable loading and retryable error content composables
  - `ThrowableMessage.kt`: shared mapping from exceptions to user-facing messages
  - `UiText.kt`: UI-friendly wrapper for dynamic strings and string resources
### Domain layer
...
- Reuse `presentation/common/ScreenStateContent.kt` for shared loading and retryable error UI where possible
- User-facing labels and messages should live in `app/src/main/res/values/strings.xml`
- Use `UiText` when a ViewModel or navigation layer needs to pass a resource-backed or dynamic message to Compose
...
- The detail screen uses a Material `TopAppBar` with an auto-mirrored back icon from the material icons extended dependency
### State management
...
- Prefer resource-backed `UiText.StringResource(...)` for stable app copy
